...
if gauge==0
    ...
    Hess=[HessFF,HessVF';HessVF,HessVV]; %%hessian, equal to the fisher information matrix
    if zer==2
        CovMat=pinv(Hess); %%pseudoinverse, apparently its singular due to gauge invariance
    else
        extra=zer-2;
        CovMatPre=pinv(Hess); %%pseudoinverse, apparently its singular due to gauge invariance
