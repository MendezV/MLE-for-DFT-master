function alpha=linemin(params,conjugdir,alpharoot,  MaxPop,Nbins,Tframes, hist)
...
grad=logligrad(params ,  MaxPop,Nbins,Tframes, hist); %%gradient of the function to be minimized
...
diff=logli(params,  MaxPop,Nbins,Tframes, hist)-logli(params+alpha*conjugdir,MaxPop,Nbins,Tframes, hist); %%difference that will be minimized 
grad2=logligrad(params+ alpha*conjugdir,MaxPop,Nbins,Tframes, hist);
...
while diff<alpha*t && abs(conjugdir'*grad2)>c2*abs(conjugdir'*grad)
    alpha=alpha*tau;
    diff=logli(params,MaxPop,Nbins,Tframes, hist)-logli(params+alpha*conjugdir,MaxPop,Nbins,Tframes, hist);
    grad2=logligrad(params+ alpha*conjugdir, MaxPop,Nbins,Tframes, hist);
