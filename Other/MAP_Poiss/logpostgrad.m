function grad=logpostgrad(params ,MaxPop,Nbins,Tframes, hist)
...
NensAv=sum((ones(Nbins,1)*N').*exp(-V*N'-ones(Nbins,1)*f')./gamma(ones(Nbins,1)*(N+1)'),2)./z; %%ensemble average of our model
