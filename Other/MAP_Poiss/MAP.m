function [fmap ,Vmap,CovMatmap,fmapError,VmapError, MAPval]=MAP(alpharoot,counts,gauge,tau)
%%%%%%%%%%
%%IN
...
rootparams(1)=0; %%fixing the gauge to avoid singularity in the covariance matrix
rootparams(2)=0; %%fixing the gauge to avoid singularity in the covariance matrix
%%first iteration is steepest descent with a variable step size 
presgrad=-logpostgrad(rootparams ,  MaxPop , Nbins, Tframes, hist); %%steepest descent
alpha=lineminpost(rootparams,presgrad, alpharoot,  MaxPop, Nbins,Tframes,hist) %%finding an adecuate step size
...
tolerance=2; %%when the gradient's magnitude is smaller than this, the search stops
%counter=0;

while sqrt(logpostgrad(params ,MaxPop,Nbins,Tframes, hist)'*logpostgrad(params ,MaxPop,Nbins,Tframes, hist))>tolerance 
   conjugdir=getconjugdirmap(params,pastparams,pastconjugdir, MaxPop,Nbins,Tframes,hist);  %%preconditioner used is PR, also it can get automatic resets to steepest descent
   ...
   alpha=lineminpost(params,conjugdir,alpharoot,  MaxPop,Nbins,Tframes, hist) %%finding an adecuate step size
   ...
   sqrt(logpostgrad(params ,  MaxPop,Nbins,Tframes, hist)'*logpostgrad(params ,  MaxPop,Nbins,Tframes, hist)) %%checking the norm of the gradient
   %counter=counter+1
end
...
if gauge==0
    Vmap=params(MaxPop+2:end); %%extracting vexation at the minimum
    fmap=params(1:MaxPop+1); %%extracting frustration at the minimum
    [stderrors,CovMatmap]=getCovMat(fmap,Vmap, MaxPop,Nbins,Tframes,0); %getting the covariance matrix for the parameters in the model without gauge transformation 
    fmapError=[0;0;stderrors(1:MaxPop-1)]; %%unpacking errors
    VmapError=stderrors(MaxPop:end); %%unpacking errors
%counter
else
    N=((1:(MaxPop+1))-1)';
    Vmap=params(MaxPop+2:end)-sum(params(MaxPop+2:end))./Nbins; %%extracting vexation at the minimum
    fmap=params(1:MaxPop+1)+sum(params(MaxPop+2:end))*N./Nbins; %%extracting frustration at the minimum
    [stderrors,CovMatmap]=getCovMat(fmap,Vmap, MaxPop,Nbins,Tframes,1);  %getting the covariance matrix for the parameters in the model with gauge transformation that sets the average potential to zero
    fmapError=[0;stderrors(1:MaxPop)]; %%unpacking errors
    VmapError=stderrors(MaxPop+1:end); %%unpacking errors
end

MAPval=logpost(params , MaxPop, Nbins, Tframes, hist);
