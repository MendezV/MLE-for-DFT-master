function [f_TC, V_F, V_M, CovMat_TC, ferror_TC, V_Ferror, V_Merror, TC_MLE_like]=TC_MLE_mixed(rootparams,alpharoot,counts_M,counts_F,gauge,tau)
...
toc
   ...
   TC_MLE_like=prePar;
		   %{
			   tic
			   counter=0;
			   V_Fr=params(MaxPop+1:MaxPop+Nbins); %%extracting vexation for females
				   V_Mr=params(MaxPop+Nbins+1:end); %%extracting vexation for males
					   fr=params(1:MaxPop); %%extracting frustration
			   [Hess,CovMat]=TC_getCovMat_Broyden(fr, V_Fr, V_Mr, MaxPop_F, MaxPop_M, Nbins, Tframes,N_F, N_M, NFNMfac);
			   while sqrt((params-pastparams)'*(params-pastparams))/(MaxPop+2*Nbins)>tolerance^2 %algorith stops when parameters change on average to the 6th decimal
						  
						  conjugdir=TC_getconjugdir(params,pastparams,pastconjugdir, MaxPop_F,MaxPop_M,Nbins,Tframes,hist, N_F, N_M, NFNMfac, NexpAv_F, NexpAV_M,CovMat*grad);  %%preconditioner used is PR, also it can get automatic resets to steepest descent
						  conjugdir(1)=0; %%fixing the gauge to avoid singularity in the covariance matrix
						  conjugdir(2)=0; %%fixing the gauge to avoid singularity in the covariance matrix
						  conjugdir(2+MaxPop_M)=0; %%fixing the gauge to avoid singularity in the covariance matrix
						  conjugdir=CovMat*conjugdir;
						  
						  alpha=TC_linemin(params,conjugdir,alpharoot, MaxPop_F,MaxPop_M,Nbins,Tframes, hist, N_F, N_M, NFNMfac, NexpAv_F, NexpAV_M,CovMat*grad); %%finding an adecuate step size
						  ...
						  prePar=[prePar;params'];
								  ...
								  [paramss,Hess, CovMat,TC_MLE_like]=Broyden_TC(params, pastparams , MaxPop_F, MaxPop_M, Nbins, Tframes, hist, prePar, N_F, N_M, NFNMfac, NexpAv_F, NexpAV_M);
								  
								  toc
	
		   %}
								  
    V_F=paramss(MaxPop+1:MaxPop+Nbins); %%extracting vexation for females
    V_M=paramss(MaxPop+Nbins+1:end); %%extracting vexation for males
    fr=paramss(1:MaxPop); %%extracting frustration
