function [f_TC, V_F, V_M, CovMat_TC, ferror_TC, V_Ferror, V_Merror]=TC_MLE(rootparams,alpharoot,counts_M,counts_F,gauge,tau)
...
presgrad=-TC_logligrad(rootparams , MaxPop_F, MaxPop_M,Nbins,Tframes, hist); %%steepest descent
alpha=TC_linemin(rootparams, presgrad, alpharoot, MaxPop_F, MaxPop_M,Nbins,Tframes,hist); %%finding an adecuate step size
...
while sqrt(TC_logligrad(params ,MaxPop_F,MaxPop_M,Nbins,Tframes, hist)'*TC_logligrad(params ,MaxPop_F,MaxPop_M,Nbins,Tframes, hist))>tolerance 
   conjugdir=TC_getconjugdir(params,pastparams,pastconjugdir, MaxPop_F,MaxPop_M,Nbins,Tframes,hist);  %%preconditioner used is PR, also it can get automatic resets to steepest descent
   ...
   alpha=TC_linemin(params,conjugdir,alpharoot, MaxPop_F,MaxPop_M,Nbins,Tframes, hist) %%finding an adecuate step size
   ...
   sqrt(TC_logligrad(params ,  MaxPop_F, MaxPop_M, Nbins, Tframes, hist)'*TC_logligrad(params , MaxPop_F, MaxPop_M, Nbins, Tframes, hist)) %%checking the norm of the gradient
   %counter=counter+1
end
