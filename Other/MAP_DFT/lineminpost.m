function alpha=lineminpost(params,conjugdir,alpharoot,  MaxPop,Nbins,Tframes, hist)
...
grad=logpostgrad(params ,  MaxPop,Nbins,Tframes, hist); %%gradient of the function to be minimized
...
diff=logpost(params,  MaxPop,Nbins,Tframes, hist)-logpost(params+alpha*conjugdir,MaxPop,Nbins,Tframes, hist); %%difference that will be minimized 
grad2=logpostgrad(params+ alpha*conjugdir,MaxPop,Nbins,Tframes, hist);
...
while diff<alpha*t && abs(conjugdir'*grad2)>c2*abs(conjugdir'*grad)
    alpha=alpha*tau;
    diff=logpost(params,MaxPop,Nbins,Tframes, hist)-logpost(params+alpha*conjugdir,MaxPop,Nbins,Tframes, hist);
    grad2=logpostgrad(params+ alpha*conjugdir, MaxPop,Nbins,Tframes, hist);
