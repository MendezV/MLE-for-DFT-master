function logp=loglitest(params , MaxPop,Nbins,Tframes, histo)
...
NexpAv=histo*N; %experimental average number of flies at each bin
fmeanexp=histo*f; %experimental average frustration at each bin
loggammaav=histo*log(gamma(N+1)); %%not necessary for the minimization, could do it but sums to
%a huge number 
c=histo.*log(histo);
c(isnan(histo.*log(histo)))=0;
logp=Tframes*sum(log(z)+V.*NexpAv+fmeanexp+loggammaav)+Tframes*sum(sum(c));
