...
def g(n,mu,V):
	return -mu + V + (F(n/1.0+delta)-F(n/1.0))/delta +special.digamma(1+n)
...
print mu
mu_counter=0
for i in np.linspace(mu,10*mu,800):
	mu_counter=0
	for j in range(Nbins):
		sign1=np.sign(g(1,i,Vexation[j]))
		sign2=np.sign(g(MaxFlies-0.5,i,Vexation[j]))
		if(sign1!=sign2):
			mu_counter+=1
	if(mu_counter==Nbins):
		a=i-0.1
...
while(counter<NmaxSteps):
	...
	for i in range(Nbins):
		print np.sign(g(1,mu,Vexation[i])),np.sign(g(MaxFlies,mu,Vexation[i])),g(1,mu,Vexation[i]),g(MaxFlies,mu,Vexation[i]),mu

		nb[i] =	brentq(g, 1, MaxFlies ,args=(mu,Vexation[i]))
