...
vexERR= list(csv.reader(open(datafileErr)))[0]
Ebarvex=np.zeros(len(vexERR))
for i in range(len(vexERR)):
	Ebarvex[i]=float(vexERR[i])
...
###frustration from data
def g2(n,mu,V):
	return (-mu + V)*n + F[n] +np.log(special.gamma(1+n))


###frustration from van der waals
#def g2(n,mu,V):
#	return (-mu + V)*n -n*np.log(1-n/7.0) +np.log(special.gamma(1+n))
...
print Ferr.size , Verr.size, Nbins, len(list(set(Ebarvex)))
...
for i in range(Nbins):
	Z=np.sum(np.exp(-g2(ns,remu,Vexation[i])))
	NERROR[i]=np.sqrt(np.square(np.sum(np.square(ns-nb[i])*np.exp(-g2(ns,remu,Vexation[i])))/Z)*Ebarvex[i]*Ebarvex[i]+np.sum(np.square(ns-nb[i])*Ferr*Ferr*np.square(np.exp(-g2(ns,remu,Vexation[i]))/Z)))
