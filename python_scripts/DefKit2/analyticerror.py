...
TcorrConv/=counter						#diving by the relevant number of points that were averaged
...
Vex=np.array(Sol[0][(np.shape(ff)[1]+np.shape(AA)[0]):],(np.sqrt(Nbins),np.sqrt(Nbins)))
