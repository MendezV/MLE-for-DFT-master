...
import csv
datafile=sys.argv[1]
Nmat=list(np.array(list(csv.reader(open(datafile))), dtype=np.int))
...
for j in range(Tframes):
	for i in range(Nbins):
		...
		if(i==24):
			Nmatsim[j][i]=Nmat[j][i0+j0*side]




Nmat=list(np.array(Nmatsim).T)
#########################################
...
file_vex = open('vex_sim'+datafile, 'w')
...
file = open('ErrSimlowvex_'+datafile, 'w')
