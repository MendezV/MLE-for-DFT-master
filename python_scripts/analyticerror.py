...
#def fit(xx,R):
#	f= (1.5*(xx)/(1-R*xx)**2-1.5*xx- xx*np.log (1 - R*xx))*factor
#	return f
def fit(xx,R):
	f= R*(xx)**2
	return f
...
#def fit(xx,R,B,C,D):
#	f= - math.fabs(R)*xx*np.log(1 - 2000.0*xx/(Abin*1000.0))+B*xx*xx/(1 - 2000.0*xx/(Abin*1000.0))+C*xx*xx*xx/(1 - 2000.0*xx/(Abin*1000.0))**2
#	return f
...
en=0.17*xnew/(Abin)
#plt.ylim([-1,50])
#plt.plot(Ncont,fit2(Ncont,popt2[0],popt2[1],popt2[2]),'r-', lw=2)
#plt.plot(Ncont,fit(Ncont,popt[0]),'g-', lw=2)
#plt.errorbar(N,F-N*slope-intercept,yerr=Error[0:np.size(F)], fmt='o')
...
print np.shape(Vex)
plt.errorbar(np.arange(len(Vex)),Vex,yerr=2*Error[(np.shape(ff)[1]+np.shape(AA)[0]):], fmt='o')
