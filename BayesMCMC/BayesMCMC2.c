...
int main(int argc, char **argv){
	...
	histo=init_array(Nbins*(MaxPop+1),0.0);
	...
	/*monte carlo thermalization step*/
	 ...
	 for(i=1;i<n_steps;i++){
		 ...
		 array_normal(fprime,idum ,MaxPop+1, 1);
		 array_normal(Vprime,idum ,Nbins, 1);
		 delta_l=calculate_likelyhood(fprime,Vprime,onesNbins , N , Nfac,  MaxPop, Nbins,  Tframes, histo)-calculate_likelyhood(f,V,onesNbins , N , Nfac,  MaxPop, Nbins,  Tframes, histo);
...
print_matrix(loglikely, n_steps,1);
...
double calculate_likelyhood(double *f,double *V,double *onesNbins , int *N ,double *Nfac,int MaxPop, int Nbins, int Tframes, double *histo)
{
	...
	static double *temp0, *temp1,*temp2,*temp3,*temp4,*temp5,*temp6,*temp7,*temp8,*temp9,*temp10,*temp11;
	...
	temp7 = init_array(n_rowA,0.0); /*z*/
	temp8 = init_array(n_rowA,0.0); /*logz*/
	temp9 = init_array(n_rowA,0.0); /*fmeanexp*/
	temp10 = init_array(n_rowA,0.0); /*Nav*/
	temp11 = init_array(n_rowA,0.0); /*VNav*/
