function logp=loglikelihoodN(params ,  MaxPop, Nbins, Tframes, hist)
...
%a huge number 
%a huge number 
logp=(-Tframes*sum(log(z)+V.*NexpAv+loggammaav)-sum(sum(gammaln(Tframes*hist+1)))+Nbins*gammaln(Tframes+1)-0.5*(V'*V)/10.0-0.5*(Nbins)*log(2*pi*10))/log(10);
