function logp=loglikelihood(params , MaxPop, Nbins, Tframes, hist)
...
%a huge number 
logp=(-Tframes*sum(log(z)+V.*NexpAv+fmeanexp+loggammaav)-sum(sum(gammaln(Tframes*hist+1)))+Nbins*gammaln(Tframes+1)-0.5*(f'*f)/100.0-0.5*(V'*V)/100.0-0.5*(MaxPop+1+Nbins)*log(2*pi*100))/log(10);
